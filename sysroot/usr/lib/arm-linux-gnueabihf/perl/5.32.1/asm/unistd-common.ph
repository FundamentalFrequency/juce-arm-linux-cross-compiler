...
unless(defined(&_ASM_ARM_UNISTD_COMMON_H)) {
    ...
    eval 'sub __NR_close_range () {( &__NR_SYSCALL_BASE + 436);}' unless defined(&__NR_close_range);
    eval 'sub __NR_openat2 () {( &__NR_SYSCALL_BASE + 437);}' unless defined(&__NR_openat2);
    eval 'sub __NR_pidfd_getfd () {( &__NR_SYSCALL_BASE + 438);}' unless defined(&__NR_pidfd_getfd);
    eval 'sub __NR_faccessat2 () {( &__NR_SYSCALL_BASE + 439);}' unless defined(&__NR_faccessat2);
    eval 'sub __NR_process_madvise () {( &__NR_SYSCALL_BASE + 440);}' unless defined(&__NR_process_madvise);
}
1;
